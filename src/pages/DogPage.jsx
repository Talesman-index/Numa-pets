...
export const DogPage = ({ onNavigate, initialCategory }) => {
  ...
  const dogCategories = [
    ...
    { id: 'walk', label: 'Sortir & Promenade' },
    { id: 'care', label: 'Soin & Hygiène' },
    { id: 'play', label: 'Jeux & Occupation' },
    { id: 'comfort', label: 'Confort & Repos' }
  ];

  const dogProducts = useMemo(() => {
    return products.filter((p) => p.animal === 'dog');
  }, [products]);
  ...
  const filteredProducts = useMemo(() => {
    return dogProducts
      .filter((p) => {
        ...
        if (selectedCategory === 'care') return p.category === 'care' || p.category === 'hygiene';
  ...
  }, [dogProducts, selectedCategory, selectedSort, maxPrice, onlyInStock]);
        ...
        <div className="reveal-up" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#FFFFFF', padding: 'var(--space-4) var(--space-6)', borderRadius: 'var(--radius-xl)', marginBottom: 'var(--space-8)', flexWrap: 'wrap', gap: 'var(--space-4)', border: '1px solid rgba(0,0,0,0.06)', boxShadow: 'var(--shadow-sm)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-6)', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', fontSize: 'var(--text-xs)' }}>
              <label htmlFor="dog-price-filter" style={{ fontWeight: 700, color: 'var(--color-text-secondary)' }}>
